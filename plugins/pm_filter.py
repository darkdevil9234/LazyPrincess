...
# Born to make history @cinemaa_boxoffice !
...
async def doc(bot, update):
    ...
    ms = await update.message.edit("\n༻☬ད 𝘽𝙪𝙞𝙡𝙙𝙞𝙣𝙜 𝗖𝗶𝗻𝗲𝗺𝗮𝗮_𝗯𝗼𝘅𝗼𝗳𝗳𝗶𝗰𝗲 𝙈𝙚𝙩𝙖𝘿𝙖𝙩𝙖...")
    c_time = time.time()
    try:
    ...
    await ms.edit("三 𝘗𝘳𝘦𝘱𝘢𝘳𝘪𝘯𝘨 𝘵𝘰 𝘳𝘦𝘤𝘦𝘪𝘷𝘦 𝗖𝗶𝗻𝗲𝗺𝗮𝗮_𝗯𝗼𝘅𝗼𝗳𝗳𝗶𝗰𝗲 𝘧𝘪𝘭𝘦...︻デ═一")
    c_time = time.time() 
    try:
       if type == "document":
          await bot.send_document(
	        update.message.chat.id,
                   ...
                   caption=caption, 
                   progress=progress_for_pyrogram,
                   progress_args=( "**⎝⎝✧ ʀᴇᴄɪᴇᴠɪɴɢ ꜰɪʟᴇ ꜰʀᴏᴍ ᴄɪɴᴇᴍᴀᴀ_ʙᴏxᴏꜰꜰɪᴄᴇ ꜱᴇʀᴠᴇʀ ✧⎠⎠**",  ms, c_time))
       elif type == "video": 
...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://telegram.me/Shazam_bot_support')
        ] 
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Mere saamne jyada smart nhi banne ka buddy 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT ISSUE 🐞", url=f"https://t.me/Cinemaa_boxoffice_support")
            ],[
                InlineKeyboardButton(text=f"⚡️ ʟᴀᴛᴇꜱᴛ ᴜᴘᴅᴀᴛᴇꜱ 🦋", url=f"https://t.me/moviesworld738")
    ...
    await query.answer('♥️ Thank You ᴄɪɴᴇᴍᴀᴀ_ʙᴏxᴏꜰꜰɪᴄᴇ ♥️')
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ 2 ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/Shazam_bot_support'),
        ] 
    )
    if offset != "":
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("Hey Buddy! The requested content is currently unavailable in our database, have some patience 🙂 - our great admin will upload it as soon as possible \n             **or**\nDiscuss issue with admin here 👉  <a href='https://t.me/Cinemaa_boxoffice_support'>Discuss Here</a> ♥️ ")
        await asyncio.sleep(10)
        await k.delete()
        return
    ...
    await msg.reply("Hey Buddy! The requested content is currently unavailable in our database, have some patience 🙂 - our great admin will upload it as soon as possible \n              **or**\nDiscuss issue with admin here 👉 <a href='https://t.me/Cinemaa_boxoffice_support'>Discuss Here</a> ♥️ ",
                    reply_markup=InlineKeyboardMarkup(btn))
